...
import com.abhi.models.OrderStatusValues;

import com.abhi.models.ProductStatus;
import com.abhi.repository.OrderDao;
...
public class OrderServiceImpl implements OrderService {
		...
		if(loggedInCustomer != null) {
			...
			if(productsInCart.size()!=0) {
				if((usersCardNumber.equals(userGivenCardNumber)) 
					...
					for(Address addr : loggedInCustomer.getAddresses()) {
						if(addr.getAddressType().equalsIgnoreCase(odto.getAddressType())) {
					...
					if(deliveryAddress == null) {
						throw new OrderException("Address type not found");
					}
	...
	public Order updateOrderByOrder(OrderDTO orderdto, Integer OrderId,String token) throws OrderException,LoginException {
		...
		if(existingOrder.getCustomer().getCustomerId()==cs.getLoggedInCustomerDetails(token).getCustomerId()) {
			...
			//existingOrder.setAddress(existingOrder.getCustomer().getAddresses().get(orderdto.getAddressType()));
			Customer loggedInCustomer = cs.getLoggedInCustomerDetails(token);
			String usersCardNumber= loggedInCustomer.getCreditCard().getCardNumber();
...
//			System.out.println(loggedInCustomer);
			if((usersCardNumber.equals(userGivenCardNumber)) 
				...
				Address deliveryAddress = null;
				for(Address addr : existingOrder.getCustomer().getAddresses()) {
					if(addr.getAddressType().equalsIgnoreCase(orderdto.getAddressType())) {
						deliveryAddress = addr;
						break;
					}
				}
				...
				existingOrder.setAddress(deliveryAddress);
